...
function HMC_test_4D(NX, NY, NZ, NT, NC, β, numtrj=10, ninterval=10)

    filefooter = "$(NX)$(NY)$(NZ)$(NT)_SU$(NC)_beta$(β).txt"
    polyfile = "hmc_poly_" * filefooter
    plaqfile = "hmc_plaq_" * filefooter
    ...
    println(fp_plaq, "0 $plaq_t")
    poly = calculate_Polyakov_loop(U, temp1, temp2)
    println("0 polyakov loop = $(real(poly)) $(imag(poly))")
    println(fp_poly, "0 $(real(poly)) $(imag(poly))")
    ...
    for itrj = 1:numtrj
        ...
        if itrj % ninterval == 0
            ...
            @time plaq_t = calculate_Plaquette(U, temp1, temp2) * factor
            sumplaq += plaq_t
            println("$itrj plaq_t = $plaq_t $(sumplaq/count)")
...
function main()
    ...
    numtrj = 30000
    ninterval = 1
    HMC_test_4D(NX, NY, NZ, NT, NC, β, numtrj, ninterval)
end
main()
